...
is_quit_choice() {
  case "$1" in
    q|Q|quit|Quit|QUIT|exit|Exit|EXIT) return 0 ;;
    *) return 1 ;;
  esac
}
script_path() {
...
add_remote_client() {
  ...
  is_quit_choice "$client_ip" && exit 0
  ...
  base_port=$(printf '%s' "$base_port" | tr -d '[:space:]')
  is_quit_choice "$base_port" && exit 0
...
while :; do
  ...
  info "Type 'q' to quit."
  read -r -u 3 -p "Choose [1/2/3/q]: " MAIN_MODE || true
  MAIN_MODE=$(printf '%s' "$MAIN_MODE" | tr -d '[:space:]')
  is_quit_choice "$MAIN_MODE" && exit 0
  case "$MAIN_MODE" in
...
if [ -n "$EXISTING" ]; then
  ...
  while :; do
    ...
    info "Type 'b' to return to main menu, or 'q' to quit."
    read -r -u 3 -p "Choose [1/2/3/4/b/q]: " MODE
